...
service quietly dying, a machine overheating) go unnoticed.
...
current budget-model prices - a few pounds a month as a daily job. The
provider and model are configurable, so each team makes its own trade-off
between cost, speed and quality; a no-AI mode runs the free half alone.
